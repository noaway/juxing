...
var defaultTransport = defaultPooledTransport()
var defaultSetting = Settings{false, "Server", 3 * time.Second, 3 * time.Second, nil, nil, nil, false}
...
func newRequest(url, method string) *Request {
	...
	return &Request{
		url: url, req: &req, params: map[string]string{},
		files: map[string]string{}, setting: defaultSetting,
		resp: &resp, body: nil,
	}
}
...
// SetUserAgent sets User-Agent header field
func (r *Request) SetTrace() *Request {
	r.trace = true
	return r
}
...
// Set transport to
func (r *Request) SetDefaultTransport() *Request {
	defaultTransport.Proxy = r.setting.Proxy
	r.setting.Transport = defaultTransport
	return r
}
...
func (r *Request) SetWithContext(ctx context.Context) *Request {
...
func (r *Request) getResponse() (*http.Response, error) {
	...
	if !r.time.IsZero() {
		r.delay = time.Now().Sub(r.time)
	}
	return resp, nil
}

// Delay returns the HTTP request response time
func (r *Request) Delay() (time.Duration, error) {
	_, err := r.getResponse()
	if err != nil {
...
// ToSimpleJson gets the data from the Bytes
// and returns the SimpleJson structure
func (r *Request) ToSimpleJson() (*Json, error) {
	data, err := r.Bytes()
	if err != nil {
		return nil, err
	}
